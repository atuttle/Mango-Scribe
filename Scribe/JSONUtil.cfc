...
<cfcomponent displayname="JSONUtil" output="false">
		...
		<cfelseif ( Left(_data, 1) EQ "[" AND Right(_data, 1) EQ "]" )
			...
			<cfloop condition="#i# LTE #dataSize#">
				...
				<cfelseif (char EQ "," AND NOT inQuotes AND nestingLevel EQ 0) OR i EQ Len(_data)+1>
					...
					<cfif dataType EQ "array">
						<cfset arrayappend( ar, deserializeCustom(dataStr, arguments.strictMapping) ) />
					...
					<cfelseif dataType EQ "struct" OR dataType EQ "query" OR dataType EQ "queryByColumns">
						...
						<cfif dataType EQ "struct">
							<cfset StructInsert( st, structKey, deserializeCustom(structVal, arguments.strictMapping) ) />
						
						<!--- If query, build the query --->
						<cfelseif dataType EQ "queryByColumns">
							<cfif structKey EQ "rowcount">
								<cfset qRows = deserializeCustom(structVal, arguments.strictMapping) />
							<cfelseif structKey EQ "columns">								
								<cfset qCols = deserializeCustom(structVal, arguments.strictMapping) />
								<cfset st = QueryNew(ArrayToList(qCols)) />
								<cfif qRows>
									<cfset QueryAddRow(st, qRows) />
								</cfif>
							<cfelseif structKey EQ "data">
								<cfset qData = deserializeCustom(structVal, arguments.strictMapping) />
	...
	<cffunction 
		...
		<cfelseif IsCustomFunction(_data)>			
			<cfreturn serializeCustom( GetMetadata(_data), arguments.strictMapping) />
		...
		<cfelseif IsArray(_data)>
			...
			<cfloop from="1" to="#ArrayLen(_data)#" index="i">
				<cfset tempVal = serializeCustom( _data[i], arguments.serializeQueryByColumns, arguments.strictMapping ) />
		...
		<cfelseif IsStruct(_data)>
			...
			<cfloop from="1" to="#ArrayLen(arKeys)#" index="i">
				<cfset tempVal = serializeCustom(_data[ arKeys[i] ], arguments.serializeQueryByColumns, arguments.strictMapping ) />
				...
				<cfloop list="#columnlist#" delimiters="," index="column">
					...
					<cfloop from="1" to="#_data.recordcount#" index="i">
						...
						<cfif arguments.strictMapping AND Len(columnJavaTypes[column])>
							<cfset tempVal = serializeCustom( JavaCast(columnJavaTypes[column],_data[column][i]), arguments.serializeQueryByColumns, arguments.strictMapping ) />
						<cfelse>
							<cfset tempVal = serializeCustom( _data[column][i], arguments.serializeQueryByColumns, arguments.strictMapping ) />
						</cfif>							
						<cfset ArrayAppend(dJSONString,tempVal) />
						<cfset rowDel = ",">	
					</cfloop>
